...
class CNNImageEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Resize if needed
        if x.shape[-2:] != self.image_res:
            x = torch.nn.functional.interpolate(x, size=self.image_res, mode='bilinear')
...
def test_free(
    ...
    env_manager.setup_room(room_size=5.0, num_objects=8)
