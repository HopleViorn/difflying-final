...
def train_free_flight(
    ...
    drone = Drone('test', batch_size=batch_size, sim_steps = sim_steps * 2, sim_dt=sim_dt)
    ...
    for epoch in t:
        ...
        target_pos = torch.tensor([0.0, 0.7, 0.8], device=DEVICE).repeat(batch_size, 1)
        mid_pos = torch.tensor([0.4, 0.5, 0.4], device=DEVICE).repeat(batch_size, 1)
        ...
        for _ in range(sim_steps):
            ...
            if _== 100:
                loss_mid_pos = torch.mean(torch.norm(mid_pos - pos, dim=1, keepdim=True))
        ...
        loss = 30 * loss_mid_pos + 30 * torch.mean(dist) + loss_vel * 0.00005 + loss_vel_down * 20 + loss_extreme_angle * 20
