...
class NavigationTask:
    ...
    def transform_policy_actions_to_high_level(self, actions):
        """
        将策略输出的动作转换为高级控制输入
        ...
        """
        # 定义各个维度的最大值
        max_velocity = torch.tensor([1.0, 1.0, 1.0], device=self.device)  # 最大速度 [vx, vy, vz] (m/s)
        max_yaw_rate = torch.tensor(1.0, device=self.device)  # 最大偏航角速率 (rad/s)
    ...
    def step(self, actions):
        """执行一步模拟"""
        # 变换动作（如果需要）
        # processed_actions = self.transform_policy_actions_to_high_level(actions)
    ...
    def compute_rewards_and_dones(self):
        ...
        very_close_mask = dist_to_target < 0.2  # 距离小于 0.5 米视为非常接近
        ...
        # # 计算动作大小惩罚
        # x_action_penalty = -rp["x_absolute_action_penalty_magnitude"] * (
        #     torch.abs(last_actions[:, 0]) ** rp["x_absolute_action_penalty_exponent"]
        # )
        # z_action_penalty = -rp["z_absolute_action_penalty_magnitude"] * (
        #     torch.abs(last_actions[:, 2]) ** rp["z_absolute_action_penalty_exponent"]
        # )
        # yaw_action_penalty = -rp["yawrate_absolute_action_penalty_magnitude"] * (
        #     torch.abs(last_actions[:, 3]) ** rp["yawrate_absolute_action_penalty_exponent"]
        # )
        # action_penalty = x_action_penalty + z_action_penalty + yaw_action_penalty
        ...
        # 使用逻辑运算符而不是位运算符
        height_crash_mask = (quad_positions[:, 1] < -0.5) | (quad_positions[:, 1] > 2) | \
        ...
        close_to_target_mask = dist_to_target < 1.0  # 当距离小于1米时开始考虑速度
        ...
        # 总奖励（添加推力均衡奖励）
        rewards = pos_reward + close_to_goal_reward + getting_closer_reward + collision_penalty*0 + crash_penalty + attitude_reward + velocity_reward + thrust_balance_reward
        
        # 存储当前距离用于下一步计算
        self.prev_dist_to_target = dist_to_target.clone()
        
        # 计算完成状态
        successes = dist_to_target < 0.3  # 到达目标的阈值，距离小于 0.1 米视为成功
        episode_timeout = self.progress_buf >= self.episode_len_steps
        ...
        self.reset_buf = torch.where(
            episode_timeout | crash_mask,  # 现在坠机也会触发重置
    ...
    def log_training_stats(self, rewards):
        ...
        print(f"avg_steps: {self.progress_buf.float().mean().item():.2f} / {self.episode_len_steps}")
